...
class StateProvider extends Component<StateContextProps, StateContextState> {
  ...
  public bounds = null;
  public startPanningCoords = null;
  // velocity helpers
  ...
  handleStopPanning = () => {
    if (this.isDown) {
      ...
      if (this.velocity && this.stateProvider.enableVelocity)
  ...
  handleTouch = event => {
    ...
    if (disabled) return;
    if (panningEnabled && event.touches.length === 1)
  ...
  zoomIn = event => {
    ...
    if (!zoomingEnabled || disabled || !wrapperComponent || !contentComponent)
      return;
